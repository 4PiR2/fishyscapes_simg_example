...
for idx, image in enumerate(images):
    ...
    anomaly = detector.estimator_worker(img)
    np.save(os.path.join(save_folder, f'{basename}_anomaly'), anomaly.detach().cpu().numpy())
    segmentation = detector.estimator_image(image)['segmentation']
    np.save(os.path.join(save_folder, f'{basename}_segmentation'), np.asarray(segmentation))
